"""
analyse_BI_m205_wiener_freq.py
==============================
Variante di analyse_BI_m205_wiener.py col filtro di WIENER a lambda DIPENDENTE
DALLA FREQUENZA (optimize_filters_wiener_lambda_freq in src/analysis.py). Il kernel

    W_lambda(f) = S*(f) / ( |S(f)|^2 + lambda(f) * NPS(f) )

ha un lambda addestrabile PER OGNI frequenza (invece di un unico scalare), cosi' il
bilancio deconvoluzione-vs-reiezione del rumore viene regolato indipendentemente a
ogni frequenza. lambda(f) e' parametrizzato come exp(log_lambda) sulla meta'
indipendente dello spettro e specchiato (kernel hermitiano); viene ottimizzato
insieme ai filtri di banda f1, f2 minimizzando la stessa metrica J del BI.
lambda(f) costante riproduce il Wiener a lambda scalare; lambda(f)=1 il Wiener
standard. Nel CSV si salvano le STATISTICHE di lambda(f) (media/mediana/min/max);
la curva lambda(f) completa (meta' indipendente) viene salvata come .npy.

Tutto il resto (orchestratore/worker su cluster, CSV concorrenza-safe, beta/rho_t)
e' identico ad analyse_BI_m205_wiener.py; cambiano la stima del BI, le cartelle di
output (m205_results_wiener_freq) e il prefisso dei job (BIWF).

Oltre al CSV dei risultati (BI_results_m205_wiener_freq.csv), ogni worker salva i
filtri di banda ADDESTRATI f1, f2 e la curva lambda(f) come vettori .npy nella
cartella m205_results_wiener_freq/trained_filters/ (file f1_ch{ch}_wp{wp}.npy,
f2_ch{ch}_wp{wp}.npy, lambda_ch{ch}_wp{wp}.npy). Per f1/f2 si salva solo la META'
INDIPENDENTE dello spettro (i primi N//2+1 bin, da DC a Nyquist); il filtro completo
...
Esempi:
    python analyse_BI_m205_wiener_freq.py                                # sottomette i job e chiude
    python analyse_BI_m205_wiener_freq.py --worker --channel 71 --wp 21  # eseguito dai job
...
OUTPUT_DIR  = os.path.join(BASE_DIR, "m205_results_wiener_freq")
LOG_DIR     = os.path.join(OUTPUT_DIR, "logs")     # stdout/stderr dei job
JOBS_DIR    = os.path.join(OUTPUT_DIR, "jobs")     # script .sh temporanei
OUTPUT_CSV  = os.path.join(OUTPUT_DIR, "BI_results_m205_wiener_freq.csv")
# Cartella coi filtri di banda ADDESTRATI f1, f2 e la curva lambda(f), salvati come
# vettori .npy (uno per coppia canale/WP, nomi distinti -> nessuna concorrenza). Per
# f1/f2 si salva solo la META' INDIPENDENTE dello spettro (i primi N//2+1 bin,
# 0..Nyquist); il resto e' il mirror hermitiano (np.concatenate([h, h[-2:0:-1]])).
# lambda(f) e' gia' la meta' indipendente.
...
JOB_NAME_PREFIX   = "BIWF"    # usato per nominare i job e per il throttling via qstat
...
#   lambda_*    = statistiche di lambda(f) ottimizzato (media/mediana/min/max);
#                 la curva lambda(f) completa e' nel .npy dei filtri
CSV_FIELDNAMES = ["channel", "wp", "vbias", "signal_amp", "sigma_analytic", "SNR",
                  "beta_Hz", "rho_t", "lambda_mean", "lambda_median", "lambda_min",
                  "lambda_max", "BI", "J_final"]
...
def save_filters_npy(dirpath: str, channel, wp, f1, f2, lam_freq):
    """Salva i filtri di banda addestrati f1, f2 e la curva lambda(f) come .npy nella
    cartella dei filtri. Ogni coppia (canale, WP) scrive file con nomi distinti,
    quindi non serve alcun lock: i job non si pestano i piedi. Di f1/f2 si salva solo
    la meta' indipendente dello spettro (N//2+1 bin); il filtro completo si
    ricostruisce con full = np.concatenate([half, half[-2:0:-1]]). lambda(f) e' gia'
    la meta' indipendente (dall'ottimizzatore)."""
    ...
    np.save(os.path.join(dirpath, f"lambda_ch{channel}_wp{wp}.npy"),
            np.asarray(lam_freq).ravel())
...
def estimate_BI_for_wp(channel, wp, vbias, meanpulse, nps, signal_amp,
    ...
    # ── Optimise Wiener band-filters + lambda(f) dipendente dalla frequenza ─────
    #   Il kernel W(f) = S*/(|S|^2 + lambda(f)*NPS) e' ricostruito ad ogni step in
    #   funzione di lambda(f); f1, f2 e lambda(f) sono ottimizzati insieme min. J.
    #   lam_freq e' la meta' indipendente di lambda(f) (array numpy, N//2+1 bin).
    f1_opt, f2_opt, lam_freq, W_unit, J_values, lambda_values = \
        an.optimize_filters_wiener_lambda_freq(
            S_torch, w_torch,
    ...
    lam_freq = np.asarray(lam_freq).ravel()
    ...
    return {
        ...
        "beta_Hz": beta_Hz,
        "rho_t": rho_t,
        # Statistiche di lambda(f); la curva completa e' nel .npy dei filtri.
        "lambda_mean": float(np.mean(lam_freq)),
        "lambda_median": float(np.median(lam_freq)),
        "lambda_min": float(np.min(lam_freq)),
        "lambda_max": float(np.max(lam_freq)),
        "BI": float(BI_estimate),
        "J_final": float(J_values[-1]),
        # Filtri di banda addestrati e lambda(f) (vettori), salvati a parte come .npy
        # in FILTERS_DIR; non entrano nel BI CSV (append_row_to_csv tiene solo
        # CSV_FIELDNAMES).
        "f1": f1_opt.detach().cpu().numpy(),
        "f2": f2_opt.detach().cpu().numpy(),
        "lambda_freq": lam_freq,
    }
...
def run_worker(channel: int, wp: int):
    ...
    try:
        ...
        save_filters_npy(FILTERS_DIR, channel, wp, res["f1"], res["f2"], res["lambda_freq"])
