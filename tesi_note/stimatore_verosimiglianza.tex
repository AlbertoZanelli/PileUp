...
\end{lstlisting}
\texttt{np.clip(y1, 0, None)} è il $\max(0, c)$ della~\eqref{eq:LT}, cioè il vincolo $a\geq0$;
il massimo lungo \texttt{axis=1} è il $\max_t$; \texttt{self.R0} è $R(0) = \norm{S}^2$, calcolato
...
for j, d in enumerate(self.d_steps):
    ...
    L = np.max(np.clip(c, 0, None) ** 2, axis=2) / self.normP[j][None, :]
\end{lstlisting}
\begin{itemize}
  \item \texttt{y2} è $y(t+\dt)$ per tutti i tempi $t$ del primo impulso: lo stesso array
    \texttt{y}, spostato di \texttt{d} posizioni. Qui si vede perché la griglia dei tempi arriva
    fino a $+1.3$~ms.
  \item \texttt{c} è $c_{P_\theta}(t) = (1-r)\,y(t) + r\,y(t+\dt)$, la~\eqref{eq:cP}, per tutti gli
    eventi, tutti gli $r$ e tutti i $t$ insieme: ha forma (eventi, $r$, $t$). Gli indici
    \texttt{None} servono solo ad allineare le dimensioni.
  \item L'ultima riga è la~\eqref{eq:Ltheta}: vincolo di ampiezza positiva, massimo sul tempo,
    divisione per la norma del template di pile-up. \texttt{L} ha forma (eventi, $r$): il $\chi^2$
    spiegato da ogni pile-up con questo ritardo.
...
\begin{itemize}
  ...
  \item Costo: circa 1.3~ms per evento. Codice: classe \texttt{PileupLikelihood} in
